...
router.post('/', async (req, res) => {
    ...
    res.send({token: token, user: user, response: true});
...
router.get('/verify', async (req, res) => {
    ...
    // console.log(token)
    if(!token) return res.send({message: 'no x-auth-token in header', response: false});
    ...
    try{
        ...
        res.send({message: 'x-auth-token unverified', response: false});
    }
